...
module Feistel_Decrypt #(parameter DATA_WIDTH=128, parameter NUM_ROUNDS=10)(
    ...
    generate
        for (i = 0; i < NUM_ROUNDS; i++) begin : feistel_rounds
            ...
            // Extract lower 64 bits from reverse round key state
            assign rk_sub = {round_keys[(NUM_ROUNDS-1)-i][1][3], round_keys[(NUM_ROUNDS-1)-i][1][2], round_keys[(NUM_ROUNDS-1)-i][1][1], round_keys[(NUM_ROUNDS-1)-i][1][0],
                             round_keys[(NUM_ROUNDS-1)-i][0][3], round_keys[(NUM_ROUNDS-1)-i][0][2], round_keys[(NUM_ROUNDS-1)-i][0][1], round_keys[(NUM_ROUNDS-1)-i][0][0]};
